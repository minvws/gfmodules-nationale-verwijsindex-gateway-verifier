...
def test_application_id_is_stamped_on_all_json_formatters() -> None:
    conf = _build(syslog_path="logserver:514", application_id="nationale-verwijsindex-gateway-verifier")

    for name, formatter in conf["formatters"].items():
        if name == "plain":
            assert "application_id" not in formatter
        else:
            assert formatter["application_id"] == "nationale-verwijsindex-gateway-verifier"
